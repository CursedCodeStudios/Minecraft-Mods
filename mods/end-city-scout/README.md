...
A client-only Fabric mod for Minecraft 26.2. Records End cities and ships, checks the **ship's elytra item frame**, and reports **living shulkers observed** nearby. Integrates optionally with Xaero's Minimap and World Map, independently of Slime Scout and Monument Scout.

## Install and use

Copy `end-city-scout-<suite-version>.jar` from `build/suite` into your instance's `mods` directory with Fabric API. Use the Minecraft, Java, and Fabric versions in the root installation guide. Select the shared **Minecraft Scouts** Xaero waypoint set to see all installed scout mods together in the current dimension.
