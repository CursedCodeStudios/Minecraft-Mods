...
final class PermanentWaypoints {
    ...
    void attach(MinimapWorld world, MinimapWorldManagerIO io) { attach(world, io::saveWorld); }
    void attach(MinimapWorld world, Saver saver) { this.world = world; this.saver = saver; }
    List<Waypoint> all() {
    ...
    Waypoint find(int x, int z, Predicate<String> names) {
        ...
        for (var marker : all()) {
            ...
            if (found == null) found = marker; else remove(marker);
        }
        return found;
    }
    ...
    void save(boolean force) {
        ...
        try { saver.save(world); dirty = false; }
        catch (IOException ex) { LoggerFactory.getLogger("dev.bedrockscout").error("Cannot save permanent Xaero waypoints", ex); }
        finally {
